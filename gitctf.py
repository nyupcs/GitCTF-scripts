...
import sys
import argparse
from utils import prompt_checkout_warning, load_config, exit_main
...
def add_confirm(parser):
    parser.add_argument("--confirm", metavar="CONFIRM", type=bool,
                        default=True,
                        help="specify whether to confirm for dangerous operations (default: True)")
...
def verify_service_main(prog, options):
    ...
    #print('verifying service {}'.format(args))
    return verify_service(args.team, args.branch, args.service_port, args.host_port,
                          args.conf)
...
def verify_exploit_main(prog, options):
    ...
    args = parser.parse_args(options)
    if args.confirm:
        pass # prompt_checkout_warning(args.service_dir)
    config = load_config(args.conf)
    return verify_exploit(args.exploit, args.service_dir, args.branch,
...
def verify_injection_main(prog, options):
    ...
    add_conf(parser)
    args = parser.parse_args(options)
    return verify_injection(args.team, args.conf)
...
def submit_main(prog, options):
    ...
    add_confirm(parser)
    args = parser.parse_args(options)
    return submit(args.exploit, args.service_dir, args.branch, args.target, args.conf,
                  args.token, args.confirm)
...
def fetch_main(prog, options):
    ...
    args = parser.parse_args(options)
    config = load_config(args.conf)
    return fetch(args.team, args.issue, config, args.token)
...
def verify_main(prog, options):
    ...
    action = options[0]
    if action == 'service':
        return verify_service_main(prog + ' service', options[1:])
    elif action == 'exploit':
        return verify_exploit_main(prog + ' exploit', options[1:])
    elif action == 'injection':
        return verify_injection_main(prog + ' injection', options[1:])
    else:
...
def score_main(prog, options):
    ...
    add_conf(parser)
    args = parser.parse_args(options)
    return show_score(args.token, args.conf)
...
def hash_main(prog, options):
    ...
    add_token(parser, False)
    args = parser.parse_args(options)
    return get_hash(args.conf, args.token)
...
def setup_main(prog, options):
    ...
    add_token(parser, False)
    args = parser.parse_args(options)
    return setup_env(args.admin_conf, args.token)
...
def eval_main(prog, options):
    ...
    args = parser.parse_args(options)
    return evaluate(args.conf, args.token)
...
def exec_service_main(prog, options):
    ...
    add_service_port(parser)
    args = parser.parse_args(options)
    return exec_service(args.service_name,
                        args.service_dir,
                        args.host_port,
                        args.service_port)
...
def exec_exploit_main(prog, options):
    ...
    parser.add_argument("--timeout", metavar="SEC", required=True,
                        help="specify timeout for exploit")
    args = parser.parse_args(options)
    return exec_exploit(args.service_name, args.exploit_dir, args.ip, int(args.port), \
                        int(args.timeout))
...
def exec_main(prog, options):
    ...
    target = options[0]
    if target == 'service':
        return exec_service_main(prog + ' service', options[1:])
    elif target == 'exploit':
        return exec_exploit_main(prog + ' exploit', options[1:])
    else:
        print('Unknown action.')
        return 'Unknown action.'
...
def main(action, options):
    if action == 'help':
        ...
        return exec_main(sys.argv[0] + ' exec', options)
    elif action == 'verify':
        return verify_main(sys.argv[0] + ' verify', options)
    elif action == 'submit':
        return submit_main(sys.argv[0] + ' submit', options)
    elif action == 'fetch':
        return fetch_main(sys.argv[0] + ' fetch', options)
    elif action == 'score':
        return score_main(sys.argv[0] + ' score', options)
    elif action == 'hash':
        return hash_main(sys.argv[0] + ' hash', options)
    elif action == 'eval':
        return eval_main(sys.argv[0] + ' eval', options)
    elif action == 'setup':
        return setup_main(sys.argv[0] + ' setup', options)
    else:
        print('Unknown action.')
        return 'Unknown action.'
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print_logo()
        print_usage()
    exit_main(main(sys.argv[1], sys.argv[2:]))
